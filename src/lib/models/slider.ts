export class Slider {
  ...
  slideDown(clientAxis: number, slideContainerSize: number) {
    ...
    if (this._currentSlide == this._copyFirstSlide) {
      this._currentSlide = this._defaultCurrentSlide;
      this._previousSlide = this._defaultLastSlide;
      this._slideLocation = -(this._currentSlide * this._slideContainerSize);
    } else if (this._currentSlide == this._copyLastSlide) {
      this._currentSlide = this._defaultLastSlide;
      this._previousSlide = this._defaultLastSlide - 1;
      this._slideLocation = -(this._currentSlide * this._slideContainerSize);
    }
    this._isSlideClick = true;
    this._loc = clientAxis || 0;
  }
  slideMove(clientAxis: number) {
    if (this._isSlideClick) this._isSlideMove = true;
    this._moveLoc = this._loc - (clientAxis || 0);
  }
  slideUp() {
  ...
  calculateSlideLocation() {
    if (Math.abs(this._moveLoc / this._slideContainerSize) > 0.5) {
      if (this._moveLoc / this._slideContainerSize < 0) {
        this._slideLocation = this._slideLocation + this._slideContainerSize * (Math.round(Math.abs(this._moveLoc / this._slideContainerSize)));
      } else {
        this._slideLocation = this._slideLocation - this._slideContainerSize * (Math.round(Math.abs(this._moveLoc / this._slideContainerSize)));
      }
    }
    if (this._slideLocation < -(this._slideLength - 1) * this._slideContainerSize) this._slideLocation = -(this._slideLength - 1) * this._slideContainerSize;
    if (this._slideLocation > 0) this._slideLocation = 0;
    this.setSlideLocationIndex();
  }
  private setSlideLocationIndex() {
  ...
  get slideContainerSize(){
    return this._slideContainerSize;
  }
